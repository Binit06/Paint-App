...
class Button {
	public:
		...
		bool checkInside(int x, int y) {
			return (x >= x_pos && y >= y_pos && x < x_pos + width && y < y_pos + height);
		}
		...
		bool Pressed(int button, int state, int x, int y) {
			hovering = checkInside(x, y);
			if (button == GLUT_LEFT_BUTTON && state == GLUT_UP && hovering) {
				if (callback) {
		...
		void Display(int window_width, int window_height) {
			if (display) {
				...
				glBegin(GL_QUADS);
				glColor3f(hovering ? 0.6f : 0.2f, hovering ? 0.6f : 0.2f, hovering ? 0.6f : 0.2f);
